...
function DonateForm({username, user_wallet}: {username: string, user_wallet:string}) {
            ...
            <label className='font-bold text-gray-700 text-base lg:text-lg xl:text-xl 2xl:text-2xl'>Donate to {username} !</label>
                ...
                <div className='flex flex-col space-y-5 w-full'>
                    <div className='w-full flex '>
                        ...
                        <input type={'number'} step={'0.01'}  value={amountCrypto} onChange={(e) => {setAmountCrypto(e.currentTarget.valueAsNumber), setAmountCurrency(e.currentTarget.valueAsNumber * coinPrice)}}  className='rounded-tr-md rounded-br-md outline-transparent focus:outline-green-300 transition-all ease-in-out duration-200 w-full p-2'/>
                    </div>
                    <div className='w-full flex'>
                        ...
                        <input type={'number'} min={'1'} step={'.01'} value={amountCurrency}  onChange={(e) => {setAmountCurrency(e.currentTarget.valueAsNumber), setAmountCrypto(e.currentTarget.valueAsNumber / coinPrice)}} className='rounded-tr-md rounded-br-md outline-transparent focus:outline-green-300 transition-all ease-in-out duration-200 w-full p-2'/>
                    </div>
                    <p className='font-semibold text-gray-700 '>1 SOL = {coinPrice} {selectedCurrency.toUpperCase()}</p>
                    <button className=' bg-green-400 text-white font-medium p-2 rounded-md cursor-pointer'>Donate</button>
                    <span className='text-gray-400 text-sm'>Powered by <a target={'_blank'} href='https://www.coingecko.com/en/coins/solana' className='font-semibold text-green-400'>CoinGecko</a></span>
                </div>
        </form>
