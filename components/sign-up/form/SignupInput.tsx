...
function SignupInput({title, inputType, handleChange, inputValue, className}: InputProps) {
  return (
    <div className='relative w-full'>
        <input type={inputType ? inputType : 'text'} value={inputValue} placeholder={title || 'My Input'} className={`peer outline-none h-10 landscape:2xl:px-2 w-full placeholder-transparent border-b-2 landscape:2xl:border-2 landscape:2xl:rounded-lg border-gray-300 dark:landscape:2xl:border-transparent focus:border-b-violet-400 landscape:2xl:focus:border-violet-400 bg-transparent  dark:landscape:2xl:bg-dark-mode-background-card-color ${className} `} onChange={(e) => handleChange(e.target.value)}></input>
