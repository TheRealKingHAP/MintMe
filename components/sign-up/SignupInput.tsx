...
function SignupInput({title, inputType, handleChange, inputValue}: InputProps) {
  return (
    <div className='relative w-full'>
        <input type={inputType ? inputType : 'text'} value={inputValue} placeholder={title || 'My Input'} className={`peer outline-none h-10 w-full placeholder-transparent border-b-2 border-gray-300 focus:border-b-violet-400`} onChange={(e) => handleChange(e.target.value)}></input>
